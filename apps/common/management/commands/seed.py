...
from ._seeders import (
    clean_old_data,
    seed_base_data,
    seed_orders_and_reservations,
    seed_products_and_warehouses,
    seed_stocks,
)
    ...
    def handle(self, *args, **options):
        ...
        try:
            with transaction.atomic():
                ...
                orders_to_create = 30
                orders_count = seed_orders_and_reservations(
                    orders_count=orders_to_create
                )
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Шаг 4 завершен: Создано {orders_count} заказов со связанными позициями и резервациями."
                    )
                )

            self.stdout.write(
                self.style.SUCCESS(
                    "🎉 Сидирование успешно завершено! Все связи соблюдены."
                )
            )
            ...
            self.stdout.write(
                self.style.ERROR(
                    "База данных автоматически откачена к исходному состоянию."
                )
            )
